...
def main():
    ...
    inputsize = 20 #struct.unpack("B", r[1])[0]
    ...
    outputsize = 20 #struct.unpack("B", r[1])[0]

    # configure i/o
    print("Configure with {0} bytes input and {1} bytes output".format(inputsize, outputsize))
    EIP.registerAssembly(ethernetip.EtherNetIP.ENIP_IO_TYPE_INPUT, inputsize, 100, C1)
    EIP.registerAssembly(ethernetip.EtherNetIP.ENIP_IO_TYPE_OUTPUT, outputsize, 150, C1)
    EIP.startIO()

    C1.registerSession()
    
    config = [-32768, 0, 0, 50, 1000, 0, 1000, 30, 30, 5]
    b = bytearray(20)
    struct.pack_into('10h', b, 0, *config)
    ...
    for w in words:
        for i in range(8):
            config_list.append(w[i]=="1")
    #print(config_list)
    C1.setAttrSingle(ethernetip.CIP_OBJ_TCPIP, 4, 150, b)
    ...
    EIP.stopIO()
    return 
    C1.sendFwdOpenReq(100, 150, 0x01, torpi=5, otrpi=5)
    C1.produce()
    ...
    while True:
        try:
            ...
            break
    C1.stopProduce()
    C1.sendFwdCloseReq(100, 150, 0x01)
    EIP.stopIO()
