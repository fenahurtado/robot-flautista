...
def read_variables():
    dir = os.path.dirname(os.path.realpath(__file__)) + '/important_vars.json'
...
def get_pos_punta(x,z,alpha, offset_x=0, offset_z=0):
    global DATA
    #alpha = alpha * pi / 180
    x2 = x  + DATA["dm"] + DATA["dp"]*cos(alpha) - DATA["dq"]*sin(alpha)
    z2 = z + DATA["dp"]*sin(alpha) + DATA["dq"]*cos(alpha)
    return x2, z2
...
def get_r_theta_o(x,z,alpha):
    ...
    r = sqrt((x2 - DATA["X_F"])**2 + (z2 - DATA["Z_F"])**2)
    theta = alpha + DATA["alpha_flauta"]
    if theta > 2*pi:
        theta -= 2*pi
    if theta < 0:
        theta += 2*pi
    o = (x2 - DATA["X_F"])*tan(alpha) - (z2 - DATA["Z_F"])
    return r, theta*180/pi, o

def get_x_z_alpha(r, theta, o):
    global DATA
    theta = theta * pi / 180
    alpha = theta - DATA["alpha_flauta"]
    a = (1 + tan(alpha)**2)
    b = (-2*o*tan(alpha))
    c = (o**2 - r**2)
    if b**2 - 4*a*c < 0:
        raise Exception('Non real number')
    x2 = (-b - sqrt(b**2 - 4*a*c))/(2*a) + DATA["X_F"]
    z2 = - o + (x2-DATA["X_F"])*tan(alpha) + DATA["Z_F"]
    x = x2 - DATA["dm"] - DATA["dp"]*cos(alpha) + DATA["dq"]*sin(alpha)
    z = z2 - DATA["dp"]*sin(alpha) - DATA["dq"]*cos(alpha)
